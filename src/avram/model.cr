...
abstract class Avram::Model
  ...
  macro primary_key(*type_declaration)
# only called when multiple primary_keys
    {% pk_types = [] of SymbolLiteral %}
    {% pk_names = [] of ASTNode %}
    {% pk_ids = [] of ASTNode %}
    {% for type_dec, index in type_declaration %}
      {% pk_types << type_dec.type %}
      {% pk_names << type_dec.var.symbolize %}
      {% pk_ids << type_dec.var.id %}
      column {{ type_dec.var }} : {{ type_dec.type }}, autogenerated: true
    {% end %}
